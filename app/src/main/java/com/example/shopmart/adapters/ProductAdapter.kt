...
class ProductAdapter(private val context: Context, private val list: ArrayList<AddProductModel>) :
    ...
    override fun onBindViewHolder(holder: productViewHolder, position: Int) {
        ...
        Glide.with(context).load(data.productCoverImg)
            ...
            .into(holder.binding.shapeableImageView2)
        ...
        val content = "MRP ₹${data.productMrp}"
        holder.binding.textView11.paintFlags =
            holder.binding.textView11.paintFlags or Paint.STRIKE_THRU_TEXT_FLAG
        holder.binding.textView11.text = content

        val offPercentage = ((data.productMrp?.toFloat()!! - data.productSp?.toFloat()!!) / data.productMrp.toFloat()) * 100
        val percentageInInt = offPercentage.toInt()
        holder.binding.percentDiscount.text = ("${percentageInInt}% OFF")
